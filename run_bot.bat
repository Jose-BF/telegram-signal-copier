...
python -c "import tools.run_bot_watch as w; ok=w._regenerate_ledger(); replay=w._regenerate_replay_trades() if ok else False; audit=w._regenerate_simulation_audit() if replay else False; w._regenerate_tick_cache_status() if audit else False; w._regenerate_weekly_replay_readiness() if audit else False; w._regenerate_tick_replay_audit() if audit else False; raise SystemExit(0 if ok and replay and audit else 1)" 2>nul
...
git add -f data\tick_cache_status.json data\weekly_replay_readiness.json 2>nul
git add -f data\tick_replay_audit.jsonl data\tick_replay_status.json 2>nul
