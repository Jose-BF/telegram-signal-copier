"""
analyze_real_v2_realistic.py — Versión REALISTA del simulador.

CORRIGE el bias del simulador anterior:

  ANTES: market entraba al "extremo cercano del rango" (precio óptimo).
  AHORA: market entra al PRECIO REAL del minuto del mensaje (M1 open).
         Si ese precio está fuera del rango → la safety check del bot lo
         cancela y la señal NO opera (P/L = 0, descartada).

Esto refleja la operativa REAL del bot:
  1. Sticker llega → bot abre market a precio actual del mercado (no al rango)
  2. Texto llega con rango → si entry fuera del rango, safety cierra todo
  3. Si entry dentro del rango → coloca limit en extremo opuesto

Resultados nuevos vs los de la v1 (optimista):
  - Esperamos MENOS señales operadas (las que entran fuera de rango se descartan)
  - Esperamos MENOR P/L total (cuando entry mejor que extremo, gracias)
  - Esperamos validación más realista para go-live
"""
...
class StrategyConfig:
    ...
    n_entries: int = 1
    target_tp_index: int = 2          # índice 0-based del TP único
    be_at_tp_index: int = -1          # -1 = sin BE
    time_stop_min: int = 60


def get_market_price_realistic(sig, m1_df):
    """Devuelve el precio REAL al que el bot entraría a market (open de vela M1).

    Devuelve None si no hay barra M1 en ese minuto.
    """
    sig_dt = sig["dt"]
...
def simulate(sig, cfg, m1_df):
    """Simula UNA señal. Devuelve (pl, n_filled, status)
    status: 'OPERATED' | 'SKIPPED_OUT_OF_RANGE' | 'NO_BARS'"""
    direction = sig["direction"]
    rng = sig.get("range")
    entry_unique = sig.get("entry")

    # Precio REAL al que entraría el market (open M1 del minuto del mensaje)
    real_market_price = get_market_price_realistic(sig, m1_df)
    ...
    # ── SAFETY CHECK como hace el bot real ──
    # Si hay rango y el entry está fuera, la señal NO opera.
    if rng is not None:
        if market_entry < rng[0] or market_entry > rng[1]:
            return 0.0, 0, "SKIPPED_OUT_OF_RANGE"
    ...
    if cfg.entry_mode == "extremes" and rng is not None:
        ...
        prices = [rng[1] - i*step if direction == "BUY" else rng[0] + i*step
                  for i in range(n)]
        # market ya cubre el primer precio (aprox). Limits son los siguientes.
        # Pero solo si están al lado adverso del market_entry (igual que dca_equispaced_prices)
        for p in prices[1:]:
            adverse = (direction == "BUY" and p < market_entry) or \
                      (direction == "SELL" and p > market_entry)
            if adverse:
                positions.append({
                    "kind": "limit", "open": False, "filled": False,
                    "entry": None, "sl": sig["sl"], "tp": target_tp,
...
def evaluate(sigs, cfg, m1):
    ...
    no_bars = 0
    for s in sigs:
        pl, n, status = simulate(s, cfg, m1)
        if status == "NO_BARS":
            no_bars += 1
            continue
        if status == "SKIPPED_OUT_OF_RANGE":
            skipped += 1
            # Estas señales no aportan P/L (no se operan)
            continue
        pls.append(pl)
    if not pls:
        return None
    ...
    return {"total": sum(pls), "avg": avg, "wr": wr, "max_dd": max_dd,
            "sharpe": sharpe, "n": len(pls), "skipped": skipped, "no_bars": no_bars}
...
def main():
    ...
    print("="*92)
    print("  CANAL 1 — REALISTA (entry = M1 open del minuto del mensaje)")
    print("="*92)
    c1_configs = [
        ...
        ("extremes TP4+TS60",          StrategyConfig("extremes",    2, 3, -1, 60)),
    ...
    for name, cfg in c1_configs:
        ...
        print(f"  {name:<32} IS: {fmt(is_m)}")
        print(f"  {' '*32} OOS:{fmt(oos_m)}\n")

    print("="*92)
    print("  CANAL 2 — REALISTA")
    print("="*92)
    c2_configs = [
        ...
        ("intra_dca/2p TP4+BE@TP1",    StrategyConfig("intra_dca",   2, 3, 0, 0)),
        ("market_only TP4+BE@TP1",     StrategyConfig("market_only", 1, 3, 0, 0)),
        ("intra_dca/4p TP3+BE@TP1",    StrategyConfig("intra_dca",   4, 2, 0, 0)),
        ("intra_dca/4p TP4+BE+TS30",   StrategyConfig("intra_dca",   4, 3, 0, 30)),
    ]
    ...
    print("\nLeyenda: skip = señales descartadas por entry fuera de rango")
    print("Comparar con v1 (optimista): los totales aquí deberían ser MENORES")
    print("y representan la operativa REAL del bot.")
